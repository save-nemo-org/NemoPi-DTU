...
- `tools/provisioning_admin/add_device.py` — needs `azure-data-tables`; upserts a device row in the provisioning service's Azure Table Storage so a given IMEI is eligible for `/certificate` and `/onboard`. Auth via `TableClient.from_connection_string()` — set `AZURE_STORAGE_CONNECTION_STRING` once per shell session via `az storage account show-connection-string` (see the script's `--help` for the exact one-liner). Signed-in user needs `listKeys` (control plane, Contributor on the storage account / RG / subscription) to read the connection string; no data-plane RBAC role required. See `docs/provisioning.md` for context.
